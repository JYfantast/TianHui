...
parser.add_argument('--input_file_path', type=str,
                    default="E:/6LLM/6evaluation/results2/MC/DeepSeek-R1-Distill-Qwen-7B/MC.final.result.csv",
                    help='输入文件路径')
...
def main():
    ...
    # 检查candidate列中空行的数量
    empty_candidate_count = data['final'].isna().sum()
    print(f"candidate列中空行的数量: {empty_candidate_count}")

    # 处理 candidate 列中的 "</think>"
    if data['final'].str.contains('</think>').any():
        data['final'] = data['final'].apply(lambda x: x.split('</think>')[1] if '</think>' in x else x)
        print("已处理 final 列中的 '</think>' 标记。")
    else:
        print("final 列中未找到 '</think>' 标记，未进行处理。")

    # 提取hyps和refs列的数据
    hyps = data['final'].tolist()
    refs = data['syndrome'].tolist()
